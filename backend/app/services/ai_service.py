"""AI Travel Service — AWS Bedrock (primary) with Groq fallback.

Uses the Bedrock **Converse API** which provides a model-agnostic interface,
so the same code works regardless of which foundation model is selected
(Claude, Llama, Titan, Mistral, etc.).
"""
...
class TravelAIService:
    """Production travel AI service.

    Provider priority:
      1. AWS Bedrock Converse API (primary — no API key needed, IAM auth)
      2. Groq Cloud API (fallback — requires GROQ_API_KEY)
      3. Deterministic local fallback (always works, no external calls)
    """
    ...
    # ------------------------------------------------------------------ #
    # Core AI completion — Bedrock first, Groq fallback
    # ------------------------------------------------------------------ #
    ...
    def _invoke_bedrock_converse(self, prompt: str, max_retries: int = 2) -> dict[str, Any] | str:
        ...
        The Converse API works the same way regardless of which foundation model
        is configured (Claude, Llama, Titan, Mistral, etc.).
        """
        client = boto3.client(
            "bedrock-runtime",
            region_name=getattr(settings, "bedrock_region", None) or settings.aws_region,
        )

        system_prompt = (
            "You are a production travel-planning API. "
            "Return valid JSON only, with no markdown fences, no commentary."
        )
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                logger.info(
                    "Bedrock converse completed in %.2fs model=%s tokens_in=%s tokens_out=%s",
                    elapsed,
                ...
                if attempt < max_retries:
                    wait = 2 ** attempt
                    logger.warning("Bedrock throttled, retrying in %ds (attempt %d/%d)", wait, attempt + 1, max_retries)
                    time.sleep(wait)
                    continue
                raise
                ...
                if attempt < max_retries:
                    wait = 2 ** attempt
                    logger.warning("Bedrock error: %s, retrying in %ds", exc, wait)
                    time.sleep(wait)
                    continue
                raise

        raise RuntimeError(f"Bedrock failed after {max_retries + 1} attempts: {last_error}")

    async def _invoke_groq(self, prompt: str) -> dict[str, Any]:
        """Invoke Groq Cloud API as fallback."""
        headers = {
